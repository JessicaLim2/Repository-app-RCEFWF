...
@EndUserText.label: 'Projection view of workflow repo'
@UI.headerInfo: { typeName: 'Workflow', typeNamePlural: 'Workflow', 
         title: { type: #STANDARD } }
define root view entity ZC_WORKFLOW as projection on ZI_WORKFLOW as Workflow
{
    @UI.facet: [
         {
               label : 'General',
...
//    @UI: {  lineItem:   [ { position: 1 , label: 'Technical Area' }]}
    ...
    @Consumption.valueHelpDefinition: [{ entity:{name: 'ZI_WORKFLOW_TECHSUBAREA_VH', 
...
//    uict_cpi_hide,
//    uict_rfc_hide,
    @UI: {  lineItem:       [ { position: 150, label: 'Workflow Template' } ],
            fieldGroup: [{ position: 10, label:'Workflow Template', qualifier: 'Detail' }],
            selectionField: [ { position: 150 }] }
    @EndUserText.label: 'Workflow Template'
    workflow_temp,
    @UI: {  lineItem:       [ { position: 160, label: 'SAP Build Project Name' } ],
            fieldGroup: [{ position: 20, label:'SAP Build Project Name', qualifier: 'Detail' }],
            selectionField: [ { position: 160 }] }
    @EndUserText.label: 'SAP Build Project Name'
    sap_build_proj,
    @UI: {  lineItem:       [ { position: 170, label: 'File Type (If Applicable)' } ],
            fieldGroup: [{ position: 30, label:'File Type (If Applicable)', qualifier: 'Detail' }],
            selectionField: [ { position: 170 }] }
    @EndUserText.label: 'File Type (If Applicable)'
    workflow_file_type,
    @UI: {  lineItem:       [ { position: 180, label: 'Method Used' } ],
            fieldGroup: [{ position: 40, label:'Method Used', qualifier: 'Detail' }],
