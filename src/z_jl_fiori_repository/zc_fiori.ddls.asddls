...
@EndUserText.label: 'Projection view of fiori repository'
@UI.headerInfo: { typeName: 'Fiori', typeNamePlural: 'Fiori', 
         title: { type: #STANDARD } }
define root view entity ZC_FIORI as projection on ZI_FIORI as Fiori
{
    @UI.facet: [
         {
               label : 'General',
...
//    @UI: {  lineItem:   [ { position: 1 , label: 'Technical Area' }]}
    ...
    @Consumption.valueHelpDefinition: [{ entity:{name: 'ZI_FIORI_TECHSUBAREA_VH', 
...
//    uict_cpi_hide,
//    uict_rfc_hide,
    @UI: {  lineItem:       [ { position: 140, label: 'UI Repository' } ],
            fieldGroup: [{ position: 140, label:'UI Repository', qualifier: 'Detail' }],
            selectionField: [ { position: 140 }] }
