CLASS lhc_ZI_ENHANCEMENT DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zi_enhancement RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zi_enhancement RESULT result.

    METHODS draftToActive FOR MODIFY
      IMPORTING keys FOR ACTION zi_enhancement~draftToActive RESULT result.

    METHODS setDefaultTechArea FOR DETERMINE ON SAVE
      IMPORTING keys FOR zi_enhancement~setDefaultTechArea.

    METHODS validateClientProj FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateClientProj.

    METHODS validateDeveloper FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateDeveloper.

    METHODS validateFunctional FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateFunctional.

    METHODS validateModuleName FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateModuleName.

    METHODS validateProject FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateProject.

    METHODS validateTechSubarea FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_enhancement~validateTechSubarea.

ENDCLASS.

CLASS lhc_ZI_ENHANCEMENT IMPLEMENTATION.
...
*   Modify the entities with required fields.
    MODIFY ENTITIES OF zi_enhancement IN LOCAL MODE
      ENTITY zi_enhancement
...
* EXECUTE Active only on draft instances.
        ...
        MODIFY ENTITIES OF ZI_enhancement IN LOCAL MODE
          ...
          REPORTED DATA(activate_enhancemented)
...
* Read the active instance to send back to Enhancement App.
    READ ENTITIES OF ZI_enhancement IN LOCAL MODE
        ENTITY zi_enhancement
        ALL FIELDS WITH CORRESPONDING #( lt_keys )
        RESULT DATA(lt_enhancement).

*    result = VALUE #( FOR ls_repository IN lt_repository ( %tky   = ls_repository-%tky
*                                                   %param = CORRESPONDING #( ls_repository ) ) ).

    result = VALUE #( for ls_enhancement in lt_enhancement ( %tky-%key = ls_enhancement-%key
                                                      %tky-uuid = ls_enhancement-uuid
                                                      %tky-%is_draft = if_abap_behv=>mk-on
                                                      %key = ls_enhancement-%key
                                                      %param = CORRESPONDING #( ls_enhancement ) ) ).
...
*                                                      %param-%key = <fs_new_key>-%key ) ).
    ...
    mapped-zi_enhancement = CORRESPONDING #( lt_enhancement ).
  ENDMETHOD.
...
**
    ...
    REPORTED DATA(lt_enhancemented).
  ...
  " Read relevant project instance data
    ...
    lt_clientproj_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING client_proj = client_prj EXCEPT * ).
    ...
    LOOP AT lt_enhancement INTO DATA(lwa_enhancement).
    " Clear state messages that might exist
      APPEND VALUE #(  %tky        = lwa_enhancement-%tky
                       %state_area = 'VALIDATE_CLIENTPJ' )
        TO reported-zi_enhancement.

      IF lwa_enhancement-client_prj IS INITIAL OR NOT line_exists( lt_clientproj[ client_proj = lwa_enhancement-client_prj ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
  ...
  " Read relevant developer instance data
    ...
    lt_Developer_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING consultant = developer EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_enhancement-developer IS NOT INITIAL AND NOT line_exists( lt_Developer[ consultant = lwa_enhancement-developer ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
  ...
  " Read relevant functional instance data
    ...
    lt_Function_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING consultant = functional EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_enhancement-functional IS NOT INITIAL AND NOT line_exists( lt_Function[ consultant = lwa_enhancement-functional ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
  ...
  " Read relevant module instance data
    ...
    lt_moduleName_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING module_name = module_name EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_enhancement-module_name IS NOT INITIAL AND NOT line_exists( lt_moduleName[ module_name = lwa_enhancement-module_name ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
        ...
        APPEND VALUE #(  %tky        = lwa_enhancement-%tky
                         ...
                         %msg = new_message(  id = 'Z_MSG_REPOSITORY'
                                              number   = '001' " &1 unknown
                                              v1 = 'Module Name'
                                              v2 = lwa_enhancement-module_name
                                              severity = if_abap_behv_message=>severity-error )
  ...
  " Read relevant project instance data
    ...
    " Optimization of DB select: extract distinct non-initial project
    lt_project_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING project = project EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_enhancement-project IS INITIAL OR NOT line_exists( lt_project[ project = lwa_enhancement-project ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
  ...
  " Read relevant project instance data
    ...
    " Optimization of DB select: extract distinct non-initial project
    lt_techSubarea_temp = CORRESPONDING #( lt_enhancement DISCARDING DUPLICATES MAPPING tech_subarea = tech_subarea EXCEPT * ).
    ...
    " Check if project exist
      SELECT FROM zdt_tech_subarea FIELDS tech_subarea
        ...
        WHERE tech_area = 'Enhancement' AND tech_subarea = @lt_techSubarea_temp-tech_subarea
    ...
    " Clear state messages that might exist
      ...
      IF lwa_enhancement-tech_subarea IS INITIAL OR NOT line_exists( lt_techSubarea[ tech_subarea = lwa_enhancement-tech_subarea ] ).
        APPEND VALUE #(  %tky = lwa_enhancement-%tky ) TO failed-zi_enhancement.
        ...
        APPEND VALUE #(  %tky        = lwa_enhancement-%tky
                         ...
                         %msg = new_message(  id = 'Z_MSG_REPOSITORY'
                                              ...
                                              v2 = lwa_enhancement-tech_subarea
                                              severity = if_abap_behv_message=>severity-error )
