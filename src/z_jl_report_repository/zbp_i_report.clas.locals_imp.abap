CLASS lhc_ZI_REPORT DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zi_report RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zi_report RESULT result.

    METHODS draftToActive FOR MODIFY
      IMPORTING keys FOR ACTION zi_report~draftToActive RESULT result.

    METHODS hideTechSubarea FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zi_report~hideTechSubarea.

    METHODS setDefaultTechArea FOR DETERMINE ON SAVE
      IMPORTING keys FOR zi_report~setDefaultTechArea.

    METHODS validateClientProj FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateClientProj.

    METHODS validateDeveloper FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateDeveloper.

    METHODS validateFunctional FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateFunctional.

    METHODS validateModuleName FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateModuleName.

    METHODS validateProject FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateProject.

    METHODS validateTechSubarea FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_report~validateTechSubarea.
...
*   Modify the entities with required fields.
    MODIFY ENTITIES OF zi_report IN LOCAL MODE
      ENTITY zi_report
...
* Read the active instance to send back to Fiori App.
    READ ENTITIES OF ZI_REPORT IN LOCAL MODE
        ENTITY zi_report
...
*                                                   %param = CORRESPONDING #( ls_repository ) ) ).
    ...
    result = VALUE #( for ls_report in lt_report ( %tky-%key = ls_report-%key
                                                      %tky-uuid = ls_report-uuid
                                                      %tky-%is_draft = if_abap_behv=>mk-on
                                                      %key = ls_report-%key
                                                      %param = CORRESPONDING #( ls_report ) ) ).
...
*                                                      %param-%key = <fs_new_key>-%key ) ).
    ...
    mapped-zi_report = CORRESPONDING #( lt_report ).
...
**
  ...
  " Read relevant project instance data
    ...
    lt_clientproj_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING client_proj = client_prj EXCEPT * ).
    ...
    LOOP AT lt_report INTO DATA(lwa_report).
    " Clear state messages that might exist
      APPEND VALUE #(  %tky        = lwa_report-%tky
                       %state_area = 'VALIDATE_CLIENTPJ' )
        TO reported-zi_report.

      IF lwa_report-client_prj IS INITIAL OR NOT line_exists( lt_clientproj[ client_proj = lwa_report-client_prj ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
  ...
  " Read relevant developer instance data
    ...
    lt_Developer_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING consultant = developer EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_report-developer IS NOT INITIAL AND NOT line_exists( lt_Developer[ consultant = lwa_report-developer ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
  ...
  " Read relevant functional instance data
    ...
    lt_Function_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING consultant = functional EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_report-functional IS NOT INITIAL AND NOT line_exists( lt_Function[ consultant = lwa_report-functional ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
  ...
  " Read relevant module instance data
    ...
    lt_moduleName_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING module_name = module_name EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_report-module_name IS NOT INITIAL AND NOT line_exists( lt_moduleName[ module_name = lwa_report-module_name ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
  ...
  " Read relevant project instance data
    ...
    " Optimization of DB select: extract distinct non-initial project
    lt_project_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING project = project EXCEPT * ).
    ...
    " Clear state messages that might exist
      ...
      IF lwa_report-project IS INITIAL OR NOT line_exists( lt_project[ project = lwa_report-project ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
  ...
  " Read relevant project instance data
    ...
    " Optimization of DB select: extract distinct non-initial project
    lt_techSubarea_temp = CORRESPONDING #( lt_report DISCARDING DUPLICATES MAPPING tech_subarea = tech_subarea EXCEPT * ).
    ...
    " Check if project exist
      SELECT FROM zdt_tech_subarea FIELDS tech_subarea
        ...
        WHERE tech_area = 'Report' AND tech_subarea = @lt_techSubarea_temp-tech_subarea
    ...
    " Clear state messages that might exist
      ...
      IF lwa_report-tech_subarea IS INITIAL OR NOT line_exists( lt_techSubarea[ tech_subarea = lwa_report-tech_subarea ] ).
        APPEND VALUE #(  %tky = lwa_report-%tky ) TO failed-zi_report.
